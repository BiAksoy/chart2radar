...
class RadarChartPlotter:
    ...
    def plot_single_player_radar(self, 
        ...
        labels, values = self.prepare_data_for_radar(zone_percentages)
    ...
    def plot_comparison_radar(self, 
        ...
        for i, (player_name, zone_percentages) in enumerate(player_data.items()):
            labels, values = self.prepare_data_for_radar(zone_percentages)
    ...
    def plot_detailed_comparison(self, 
                                player_data: Dict[str, Dict[str, float]],
                                title: str = "Detailed Player Comparison") -> plt.Figure:
        ...
        for i, (player_name, zone_percentages) in enumerate(player_data.items()):
            values = [zone_percentages.get(zone, 0.0) for zone in zones]
            color = self.colors[i % len(self.colors)]
